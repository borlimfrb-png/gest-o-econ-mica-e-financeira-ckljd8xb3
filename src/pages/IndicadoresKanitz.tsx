...
export default function IndicadoresKanitz() {
  ...
  // Cor e Ícone do Semáforo
  const statusConfig = useMemo(() => {
    switch (kanitzAtual.classificacao) {
      case 'solvente':
        return {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-16">
      ...
      {/* 2. CARD HERO DO TERMÔMETRO DE KANITZ & SEMÁFORO VISUAL */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        {/* GAUGE / TERMÔMETRO HORIZONTAL VISUAL */}
        <Card className="lg:col-span-7 bg-white border-slate-200 shadow-2xs flex flex-col justify-between">
            ...
            <div className="flex items-center justify-between">
              ...
              {kanitzAtual.fi !== null && (
                <Badge className="bg-slate-100 text-slate-800 font-mono text-xs font-bold border-slate-200">
                  FI:{' '}
                  {kanitzAtual.fi > 0 ? `+${kanitzAtual.fi.toFixed(2)}` : kanitzAtual.fi.toFixed(2)}
                </Badge>
              )}
            </div>
          </CardHeader>
          ...
          <CardContent className="p-5 space-y-6">
            {/* Termômetro Horizontal em Barra com Marcador Dinâmico */}
            <div className="space-y-2 pt-4">
              <div className="relative w-full">
                {/* Marcador em Ponteiro com Tooltip Flutuante */}
                {kanitzAtual.fi !== null && (
                  <div
                    className="absolute -top-7 transition-all duration-500 -translate-x-1/2 flex flex-col items-center z-10"
                    style={{
                      left: `${Math.max(3, Math.min(97, kanitzAtual.termometroPosicaoPercentual))}%`,
                    }}
                  >
                    <div className="bg-[#0B1F3A] text-white text-[10px] font-bold font-mono px-2 py-0.5 rounded-md shadow-md whitespace-nowrap">
                      FI {kanitzAtual.fi.toFixed(2)}
                    </div>
                    <div className="w-2 h-2 bg-[#0B1F3A] rotate-45 -mt-1" />
                  </div>
                )}
                ...
                */}
                ...
                {/* Marcador de Linha Vertical sobre a Barra */}
                {kanitzAtual.fi !== null && (
                  <div
                    className="absolute top-0 bottom-0 w-1 bg-slate-950 rounded-full shadow-lg transition-all duration-500 -translate-x-1/2 pointer-events-none"
                    style={{
