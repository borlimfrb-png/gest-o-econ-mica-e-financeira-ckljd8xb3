...
export default function Centros() {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="outline"
            className="h-9 text-xs font-semibold border-slate-200 hover:bg-blue-50 hover:text-blue-700 text-[#0B1F3A] shadow-xs"
            title="Consultar e verificar contas cadastradas na empresa"
          >
            <SearchCheck className="w-4 h-4 mr-1.5 text-blue-600" />
            Consultar Contas
            <Badge
              variant="secondary"
              className="ml-2 text-[10px] px-1.5 py-0 h-4 bg-slate-100 text-slate-700"
            >
              {contas.length}
            </Badge>
          </Button>
                            ...
                            <td className="py-3 px-4 text-right">
                              <div className="flex items-center justify-end gap-1.5">
                                <Button
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    setSelectedCentroId(c.id)
                                    setConsultarContasOpen(true)
                                  }}
                                  size="sm"
                                  variant="ghost"
                                  className="h-7 w-7 p-0 text-slate-500 hover:text-blue-600 hover:bg-blue-50"
                                  title="Verificar contas cadastradas vinculadas a este centro"
                                >
      ...
      {/* ============ MODAL CONSULTA DE CONTAS CADASTRADAS ============ */}
      <ModalConsultarContas
        open={consultarContasOpen}
        onOpenChange={setConsultarContasOpen}
        contas={contas}
        centros={centros}
        lancamentos={lancamentos}
        centroSelecionado={selectedCentro}
        onSelecionarContaParaLancamento={(contaId) => {
          setLancField('conta', contaId)
          toast({
