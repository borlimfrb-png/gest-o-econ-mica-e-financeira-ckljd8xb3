...
import type { ContaRecord, TipoConta } from '@/types/finance'
...
import { BookOpen, Plus, Pencil, Trash2, AlertCircle, Search } from 'lucide-react'
...
export default function Contas() {
  ...
  const loadData = async () => {
    try {
      setLoading(true)
      const list = await contasService.getAll()
      setContas(list)
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[11px] text-slate-400">
                    Sugestões conforme o tipo. Você também pode digitar um grupo personalizado na
                    edição.
                  </p>
                </div>
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="conta-descricao" className="text-xs font-semibold text-slate-700">
                  Descrição
                </Label>
                <Input
                  id="conta-descricao"
